...
dropVarsIn [] (InResVar el z) = dropVarsIn (dropEntry [] el) z
dropVarsIn (x :: y) (InResVar el z) = dropVarsIn (dropEntry (x :: y) el) z
...
mkComposite [] (InResVar el z) =  CompCons (getVarEntry [] el) (mkComposite (dropEntry [] el) z)
mkComposite (x :: y) (InResVar el z) = CompCons (getVarEntry (x :: y) el) (mkComposite (dropEntry (x :: y) el) z)
...
keepEnv [] (InRes el prf) = keepEnv (dropDups [] el) prf
keepEnv (x :: y) (InRes el prf) = keepEnv (dropDups (x :: y) el) prf
...
rebuildEnv [] (x :: xs) (Skip p) = x :: rebuildEnv [] xs p
rebuildEnv (y :: z) (x :: xs) (Skip p) = x :: rebuildEnv (y :: z) xs p

runST : Env invars -> STrans m a invars outfn ->
        ((x : a) -> Env (outfn x) -> m b) -> m b
runST env (Pure {out_fn} result) k = k result env
runST env (Bind prog next) k
   = runST env prog (\prog', env' => runST env' (next prog') k)
runST env (Lift action) k
   = do res <- action
        k res env
runST env (RunAs {m} prog) k = runST env prog (\res, env' => k (pure res) env')
...
runSTLoop (More x) env (Pure {out_fn} result) k onDry = k result env
...
export
ConsoleIO IO where
  putStr str = lift (PrimIO.putStr str)
  getStr = lift PrimIO.getLine

{-
  putChar c = lift (PrimIO.putChar c)
  getChar = lift PrimIO.getChar
-}
...
ConsoleIO (IOExcept err) where
  putStr str = lift (ioe_lift (PrimIO.putStr str))
  getStr = lift (ioe_lift PrimIO.getLine)

  putChar c = lift (ioe_lift (PrimIO.putChar c))
  getChar = lift (ioe_lift PrimIO.getChar)
-}
